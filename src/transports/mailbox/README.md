...
- `rpc-fn`: **(强制)** 工具/函数标识符 (Function ID)。
- `rpc-act`: **(强制)** 操作动作 (Action)，如 `get`, `post`, `delete`, `list` 等。默认为 `post`。
- `req-id`: **(强制)** 请求的唯一关联 ID (Correlation ID)。
- `mbx-reply-to`: 响应应发往的目标邮箱地址（默认为请求的 `from`）。
- `rpc-res-id`: **(可选)** 资源标识符 (Resource ID)，用于标识特定的资源实例。

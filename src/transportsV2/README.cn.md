...
### 2.3 RpcError (推荐错误处理)
为了确保错误能被正确序列化并在客户端还原，推荐使用 `RpcError`：
```typescript
...
* **clearSchemes(scheme?)**：清空注册表（主要用于单元测试隔离）。
...
| :--- | :--- | :--- |
| `RPC_HEADERS.TOOL_ID` | `rpc-fn` | 目标工具 ID |
| `RPC_HEADERS.RES_ID` | `rpc-res-id` | 目标资源 ID |
| `RPC_HEADERS.ACT` | `rpc-act` | 目标动作 Action (以 `$` 开头表示内置) |
| `RPC_HEADERS.TRACE_ID` | `trace-id` | 链路追踪 ID |
| `RPC_HEADERS.TIMEOUT` | `rpc-timeout` | 客户端声明的最大容忍时间 (ms) |
| - | `req-id` | 必须在响应中原样返回的 ID |
...
2. **工具查找 (两级策略)**：
   * 优先从用户自定义 `registry` 匹配 `toolId`。
   * 若未找到且为系统工具，则从内置 `systemRegistry` 查找（支持 `rpcTask` 自动挂载）。
   * 若均不存在，返回 `404 Not Found`。
...
7. **响应回填**：强制在所有响应（成功/失败/处理中）的 Header 中回填 `req-id`。
...
* **瀑布流寻址 (Waterfall Routing)**：
  * 优先检测 Header 中的显式指令。
  * 降级解析 URL Path：`/api/foo/bar` -> `toolId: foo`, `resId: bar`。
* **HTTP 状态码映射**：
...
* **路由不匹配**：检查 Header 中的 `rpc-fn` 是否与注册名一致，或者 URL 路径前缀是否配置正确。
* **408 超时**：检查 `rpc-timeout` 设置是否过短，或工具是否应该开启 `keepAliveOnTimeout` 以允许 102 转换。
...
2. **重命名 Header** (如果使用自定义 HTTP 请求)：
   * `rpc-id` -> `rpc-res-id` (旧名仍作为别名支持，但建议替换)。
3. **方法名重构**：

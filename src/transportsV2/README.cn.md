...
| :--- | :--- | :--- |
| `RPC_HEADERS.TOOL_ID` | `rpc-func` | 目标工具 ID |
| `RPC_HEADERS.RES_ID` | `rpc-res-id` | 目标资源 ID |
| `RPC_HEADERS.ACT` | `rpc-act` | 目标动作 Action (以 `$` 开头表示内置) |
| `RPC_HEADERS.TRACE_ID` | `rpc-trace-id` | 链路追踪 ID |
| `RPC_HEADERS.TIMEOUT` | `rpc-timeout` | 客户端声明的最大容忍时间 (ms) |
| - | `req-id` | 必须在响应中原样返回的 ID |

> [!TIP]
> **Header 优先级**：在服务端（如 MailboxServer）解析请求时，V2 协议头 (`rpc-func`) 的优先级始终高于 V1 协议头 (`mbx-fn-id`)。
...
*   **Header 映射与互操作**：
    ...
    *   **优先级策略**：当物理消息中同时存在 `rpc-` 与 `mbx-` 标识时，始终优先采用 V2 标准头。
...
*   **路由不匹配**：检查 Header 中的 `rpc-func` 是否与注册名一致，或者 URL 路径前缀是否配置正确。
...
4.  **Mailbox 参数对齐**：
    ...
    *   **Header 演进**：如果外部系统依赖 `mbx-fn-id`，服务端仍会识别，但建议新项目优先使用标准 `rpc-func`，以获得跨协议的一致性。
